...
<header class="flex flex-col md:flex-row justify-between items-center mb-10 md:mb-16 px-6 md:px-12 py-8 md:pt-10 md:pb-0 gap-6 md:gap-0">
    ...
    <!-- Search Bar -->
    <div class="flex w-full md:w-search-bar bg-white/5 border border-white/5 backdrop-blur-xl rounded-2xl items-center gap-3 px-5 py-3.5 text-muted-text transition-all duration-300 shadow-lg shadow-black/20 order-3 md:order-2">
    ...
    <div class="flex items-center justify-between md:justify-end w-full md:w-auto gap-5 order-2 md:order-3">
        <button wire:click="toggleNotifications"
            class="hidden md:flex w-12 h-12 bg-white/5 border border-white/5 backdrop-blur-xl rounded-2xl flex items-center justify-center text-muted-text hover:bg-white/10 hover:text-white transition-all duration-300 relative group cursor-pointer shadow-lg shadow-black/20">
            <x-icons.nofitication.default class="w-6 h-6 {{ $hasNotifications ? 'animate-ring text-white' : '' }}" />
            @if($hasNotifications)
                <span class="absolute top-3.5 right-3.5 w-2 h-2 bg-primary-blue rounded-full shadow-active-glow"></span>
            @endif
        ...
        <!-- User Profile -->
        <button
            class="flex items-center flex-1 md:flex-none gap-3 bg-white/5 border border-white/5 backdrop-blur-xl rounded-2xl p-1.5 pr-4 hover:bg-white/10 transition-all duration-300 group cursor-pointer shadow-lg shadow-black/20">
            <div
